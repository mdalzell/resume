\documentclass[12pt]{article}
\usepackage{paracol}
\usepackage[margin=0.9in]{geometry}

\renewcommand{\familydefault}{\sfdefault}
\begin{document}
\pagenumbering{gobble}

\section*{Matt Dalzell}

\columnratio{0.30}
\setlength{\columnsep}{4em}
\setlength{\columnseprule}{0.1pt}
\begin{paracol}{2}

% Contact Info
\subsection*{Contact Info}
contact@mattdalzell.com \\ 
https://mattdalzell.com \\
https://github.com/mdalzell

% Technical Skills
\subsection*{Technical Skills} 

% Languages
\textbf{Languages}
\begin{itemize}
    \item TypeScript 
    \item JavaScript/ES6
    \item C\#
\end{itemize}

% Frameworks
\noindent \textbf{Libraries \& Frameworks}
\begin{itemize}
    \item React 
    \item Electron
    \item .NET 
\end{itemize}

% Tools
\noindent \textbf{Tools}
\begin{itemize}
    \item Visual Studio Code
    \item npm
    \item git
\end{itemize}

% Testing
\noindent \textbf{Testing}
\begin{itemize}
    \item Cypress
    \item Jest
    \item MSTest
\end{itemize}

% Software Architecture
\noindent \textbf{Software Architecture}
\begin{itemize}
    \item REST
    \item SOLID Principles
\end{itemize}

\noindent \textbf{Also familiar with:}\\
Flow, HTML5, CSS, LESS, Bootstrap, Webpack, graphql, TFS, SQL, VB.NET, JIRA

\switchcolumn 

% Professional Experience
\subsection*{Professional Experience} 

% Inspire11
\textbf{Inspire11} - Chicago, IL \\
\textit{Frontend Engineer} - {\footnotesize{(January 2019 - Present)}} 
\begin{itemize}
    \item{\footnotesize{As a consultant, worked to improve a React SPA for a fin-tech company by adding features, fixing bugs, and refactoring}}
    \item {\footnotesize{Added a Launch Darkly feature flag solution to the application using Context API and higher order components}}
    \item {\footnotesize{Collaborated as part of a remote team following agile practices and workflows.}}
\end{itemize} 

% VelocityEHS
\noindent \textbf{VelocityEHS} - Chicago, IL \\
\textit{Software Engineer II} - {\footnotesize{(November 2015 - December 2018)}}
\begin{itemize}
    \item {\footnotesize{Participated in the architecture, construction, and deployment of an Environment, Health, and Safety (EHS) SaaS application as part of a comprehensive multi-year rewrite effort.}}
    \item {\footnotesize{Helped set coding standards for the React frontend as the company integrated it into their stack.}}
    \item {\footnotesize{Leveraged React code used for the web application and the Electron framework to build a fully featured Windows desktop application, both as a native installed app and a portable executable.}}
    \item {\footnotesize{Assisted in architecting and building the .NET backend while using SOLID principles}}
\end{itemize} 

% Azavar Technologies
\noindent \textbf{Azavar Technologies} - Chicago, IL \\
\textit{Web Developer} - {\footnotesize(June 2013 - November 2015)}
\begin{itemize}
    \item {\footnotesize{Built a website for a staffing firm using a Bootstrap responsive layout. Integrated with third party APIs and a .NET CMS so that administrators could manage custom content.}}
    \item {\footnotesize{Worked with a team to build an eLearning application for middle and high school student using the .NET MVC framework. }} 
\end{itemize}

% Education
\subsection*{Education}
\textbf{Northwestern University} - Evanston, IL \\
\textit{B.S. in Computer Science} - {\footnotesize{Class of 2013}}

% Other Honors
\subsection*{Other Honors}
\textbf{Eagle Scout} \\
\textit{Troop 652, Boy Scouts of America} - {\footnotesize{2009}}
    
\end{paracol}
\end{document}